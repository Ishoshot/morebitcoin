        ...
        <div class="px-sm-2 px-md-0 sp-mb-10 sp-mt-5">
          <div class="container-fluid shadow border py-3 px-sm-3 px-lg-2 mt-3">
            ...
            <b-table
              ...
              :fixed="false"
              :sticky-header="false"
              :busy="!paginatedAdminInvestments.data"
              :sort-by.sync="sortBy"
              :sort-desc.sync="sortDesc"
              :items="paginatedAdminInvestments.data"
              :fields="fields"
              :selectable="true"
              :select-mode="'single'"
              ref="selectableTable"
            ...
            <div class="small text-left">
              Number of items:
              <b>{{
                !paginatedAdminInvestments.data
                  ? ""
                  : paginatedAdminInvestments.data.length
              }}</b>
            </div>

            <div class="d-flex justify-content-center mt-2">
              <pagination
                :data="paginatedAdminInvestments"
                @pagination-change-page="getAdminInvestments"
              >
...
export default {
  ...
  computed: {
    ...mapGetters(["paginatedAdminInvestments"]),
    sortBy_() {
  ...
  created() {
    this.getAdminInvestments();
  },

  methods: {
    ...mapActions(["fetchAdminInvestments"]),

    async getAdminInvestments(page) {
      if (typeof page === "undefined") {
        page = 1;
      }
      await this.fetchAdminInvestments(page);
    },

    onSelect(item) {
      this.selected = item;
      this.$router.push({
        path: "/admin/investment/" + this.selected[0].id,
      });
    },

    choose(item) {
      return this.$router.push({
        path: "/admin/investment/" + item,
      });
    },
  },
};
